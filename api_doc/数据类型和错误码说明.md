...
code 采用的是 http 状态码方式，2xx 表示成功；4xx 表示客户端错误；5xx 表示服务端错误
...
| × | _419_     | Page Expired                    | 临时token已过期，页面已失效 |         | 【自定义】Laravel框架已采用。如CSRF、refresh_token或其他临时token不存在或已过期                          |
| × | _420_     | EnhanceYourCalm                 | 客户端被限速           |         | 【自定义】Twitter和Trends API已采用。                                                     |
| × | **423**   | Locked                          | 资源被锁定            |         |                                                                                 |
...
| × | **440**   | Login Time-out                  |                  |         | 客户端 cookie或access_token 超时失效，需要重新登录                                             |
| × | _444_     | No Response                     | 无反应              |         | 【自定义】Nginx已采用。检测到恶意软件，服务器不向客户端返回任何信息，并关闭连接                                      |
| × | **451**   | Unavailable For Legal Reasons   | 内容违反规定           |         | 该访问因法律的要求而被拒绝，由IETF在2015核准后新增加                                                  |
| × | _490~499_ |                                 |                  |         | 自定义错误码                                                                          |
| × | _494_     | No Rows Available               | 无符合条件的数据         | 200/204 | 【自定义】查询列表，没有匹配的数据列表。不同于 {code:200, data:[]} 这种情况是数据被过滤掉了，还可以继续访问下一页；而494则没有下一页了 |
...
| × | _590_     | Exception                       |                  |         | 【自定义】网关层拦截导致错误，未达到业务程序阶段。一般由路由层，或nginx等代理层抛出                                    |